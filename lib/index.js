...
function runChannel(ctx, config, logger) {
  ...
  async function acquireAuthCookie() {
    try {
      ...
      // 诊断用：把 launch token 记到本地日志，便于离线复现 /api 行为（仅本机可读文件）
      fileLog(`auth-debug: token=${token}`);
      return cookie;
    } catch (err) {
  ...
  const V2_MAP = {
    ...
    'session.prompt': (p = {}) => ({ path: 'session/prompt', args: { request: { requestId: randomUUID(), ...p } } }),
    'session.page':   (p = {}) => ({ path: 'session/page',   args: { request: p } }),
  };

  async function postRpc(endpoint, method, payload, signal) {
    ...
    try {
      ...
      const why = err?.name === 'TimeoutError' ? `timeout after 15s` : (err?.message ?? err);
  ...
  async function rpc(method, payload, signal, retriedAuth = false) {
    ...
    if (protocol === null) throw new Error(`rpc ${method}: no usable DSH RPC protocol`);
    try {
      if (protocol === 'v2') {
        ...
        return await postRpc(endpoint, endpoint, { args }, signal);
      }
      return await postRpc(method, method, payload, signal);
    } catch (err) {
      if (err?.status === 401 && !retriedAuth) {
        // cookie 过期或首次未取到：重新认证一次再试
  ...
  /** 撤回一条已发送的消息（审批/提问处理完就撤，免得按钮还能再点）。 */
  ...
  async function handleInteraction(d) {
    ...
    const ai = approvalOrder.indexOf(rpcId);
    if (ai !== -1) approvalOrder.splice(ai, 1);
  ...
  function handleFrame(frame) {
    ...
    if (ev.type === 'approval/decided') {
      ...
      for (const [rid, e] of pendingApprovals) {
        if (e.approvalId === decidedId) {
          pendingApprovals.delete(rid);
          const oi = approvalOrder.indexOf(rid);
          if (oi !== -1) approvalOrder.splice(oi, 1);
  ...
  function handleControlItem(value) {
    ...
    // v2 事件帧形态：广播为 {type:'emit', event:'<name>', args:[...]}；
    // 需要客户端应答的 waterfall 转发会额外带 eventId（回传结果时用）。
    ...
    if (eventName === 'approval/request' || eventName === 'approval/requested') {
      ...
      handleFrame({
        ...
        payload: {
          ...
          sessionId,
          approvalId: eventId,          // v2 用事件 id 作为审批句柄
  ...
  async function pollOneSession(sid, cursors, lastSeqs, baselined) {
    ...
    const page = await rpc('session.page', { address, throughSeq: cursor, maxMessages: 40 });
  ...
  async function promptWithImageFallback(sid, content) {
    ...
    if (!hasImage) {
      return rpc('session.prompt', { sessionId: sid, mode: 'queue', content, clientTimeZone: TZ });
    }
    try {
      return await rpc('session.prompt', { sessionId: sid, mode: 'queue', content, clientTimeZone: TZ });
    } catch (err) {
      ...
      const textOnly = content.map((p) => (p?.type === 'image'
        ...
        : p));
      return rpc('session.prompt', { sessionId: sid, mode: 'queue', content: textOnly, clientTimeZone: TZ });
    }
  }
