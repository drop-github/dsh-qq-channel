...
function runChannel(ctx, config, logger) {
  ...
  // ---- DSH 事件流 ----
  // v1（≤0.1.1）：WebSocket /api/events.mux 推送；
  // v2（≥0.1.5）：events.mux 已移除，且流式方法（session/follow、session/control）
  //   必须走 /api/remote.mux 的流载体。这里改用 session/page 轮询拉取会话事件
  //   （事件对象结构与 v1 的 payload.event 一致，可直接喂给 handleFrame）。
  //   代价：审批/提问属于 control 流，轮询拿不到 —— 见 notifyControlGap()。
  async function startEventStream() {
    if (protocol === 'v2') return startEventStreamPolling();
  ...
  let controlGapNotified = false;
  function notifyControlGap() {
    if (controlGapNotified) return;
    controlGapNotified = true;
    log('v2 协议：审批/提问走 control 流（需 remote.mux 流载体），本版本暂不支持，请到 Web GUI 处理');
  }
