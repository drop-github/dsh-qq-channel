...
function runChannel(ctx, config, logger) {
  ...
  function acquireLock() {
    try {
      if (fs.existsSync(LOCK_FILE)) {
        ...
        if (cur?.pid && cur.pid !== process.pid && Date.now() - (cur.at ?? 0) < 60 * 1000) {
          try {
            process.kill(cur.pid, 0);   // 仅探测存活
            return false;               // 另一个实例还活着 → 不抢
          } catch {
            /* pid 不存在 → 锁过期，继续抢占 */
          }
        }
  ...
  async function connectQQ() {
    ...
    while (!disposed.flag) {
      ...
      // close-code 分类重连表（对齐 Hermes adapter 语义）
      if (closeCode === 4004) {
        accessToken = '';      // 清 token，下次单飞重取
        gatewaySessionId = '';
        log('QQ close 4004: clearing access token');
      } else if (closeCode === 4008 || closeCode === 4009) {
        backoff = Math.min(backoff * 2, 60000);
        log(`QQ close ${closeCode}: backoff ${backoff}ms`);
      } else if ([4001, 4002, 4003, 4005, 4006, 4007, 4010, 4011, 4012, 4013, 4014, 4914, 4915].includes(closeCode)) {
