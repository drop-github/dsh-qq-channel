...
function runChannel(ctx, config, logger) {
  ...
  // ---- 回复目标队列（落盘到 DSH_HOME/storages） ----
  const dshHome = process.env.DSH_HOME ?? '';
  const TURN_CTX_FILE = dshHome ? path.join(dshHome, 'storages', 'qq-channel-turn-ctx.json') : '';
  const CTX_STALE_MS = 10 * 60 * 1000;
  function loadTurnCtxQueue() {
    if (!TURN_CTX_FILE) return [];
    try {
      const arr = JSON.parse(fs.readFileSync(TURN_CTX_FILE, 'utf8').replace(/^\uFEFF/, ''));
      if (!Array.isArray(arr)) return [];
      const fresh = arr.filter((c) => c && typeof c === 'object' && Date.now() - (c.at ?? 0) < CTX_STALE_MS);
      if (fresh.length !== arr.length) log(`turn-ctx: 丢弃 ${arr.length - fresh.length} 条过期目标`);
      return fresh;
    } catch {
      return [];
    }
  ...
  function saveTurnCtxQueue(queue) {
    if (!TURN_CTX_FILE) return;
    try {
      fs.mkdirSync(path.dirname(TURN_CTX_FILE), { recursive: true });
  ...
  let turnBuffer = [];
  let inTurn = false;
  let turnCtxQueue = loadTurnCtxQueue();
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      try {
        ...
        const bodyFor = (md, withId) => md
          ? { msg_type: 2, markdown: { content: part }, ...(withId ? { msg_id: msgId, msg_seq: nextMsgSeq(msgId) } : {}) }
          : { content: part, msg_type: 0, ...(withId ? { msg_id: msgId } : {}) };
  ...
  function handleFrame(frame) {
    ...
    if (!p) return;
    if (p.type === 'approval/requested' && p.sessionId === targetSessionId) {
      ...
      sendText(`🔐 需要审批 #${n}：${what}\n回复「同意」批准 /「拒绝」驳回（可带编号，如：同意${n}）`, {});
      return;
    }
    if (p.type === 'approval/resolved' && p.sessionId === targetSessionId) {
    ...
    if (p.type === 'question/requested' && p.sessionId === targetSessionId) {
      ...
      if (p.questions?.length === 1 && q?.options?.length) {
        ...
        sendText('❓ 会话有提问（多问题/自定义输入），请在电脑 GUI 处理。', {});
      }
      return;
    }
    if (p.type !== 'session/event' || p.sessionId !== targetSessionId) return;
    const ev = p.event;
    if (!ev) return;
    if (ev.type === 'turn/start') { turnBuffer = []; inTurn = true; return; }
    if (ev.type === 'turn/end') {
      inTurn = false;
      const reply = turnBuffer.join('').trim();
      turnBuffer = [];
      const c = turnCtxQueue.shift() ?? lastCtx ?? {};
      saveTurnCtxQueue(turnCtxQueue);
      if (reply) sendText(reply, c, { asMarkdown: config.markdown });
      return;
    }
    if (inTurn && ev.type === 'assistant/message') {
      const t = eventText(ev);
      if (t) turnBuffer.push(t);
    }
  }
  ...
  function handleQQDispatch(t, d) {
    ...
    log(`QQ message ${t} (${c.groupOpenId ?? c.userOpenId}): ${text.slice(0, 120)}${hasImages ? ` +${images.length} image(s)` : ''}`);
    const turnCtx = { groupOpenId: c.groupOpenId, userOpenId: c.userOpenId, msgId: c.msgId, at: Date.now() };
    turnCtxQueue.push(turnCtx);
    saveTurnCtxQueue(turnCtxQueue);
    // 组装内容：文本 + 图片（下载后 base64，DSH image 块支持 png/jpeg/webp/gif）
    prepareContent(text, images)
      .then((content) => rpc('session.prompt', {
      ...
      .then((value) => {
        ...
        if (config.ack) sendText(hasImages ? `已收到 ✅（含 ${images.length} 张图片）正在处理…` : '已收到 ✅ 正在处理…', { groupOpenId: c.groupOpenId, userOpenId: c.userOpenId });
      })
      .catch((err) => {
        ...
        try {
          if (!targetSessionId) {
            ...
            if (main) {
              targetSessionId = main.sessionId;
              clearInterval(t);
              log(`bridge up: QQ -> DSH session ${targetSessionId}`);
              startEventStream();
              connectQQ();
            }
          }
