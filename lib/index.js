...
function runChannel(ctx, config, logger) {
  ...
  async function handleInteraction(d) {
    ...
    if (mq) {
      ...
      // 选定后结清并撤回原提问消息（其余选项随即失效）
      pendingQuestions.delete(qRpcId);
      const qi = questionOrder.indexOf(qRpcId);
      if (qi !== -1) questionOrder.splice(qi, 1);
      if (qEntry.msgId) recallMessage(qEntry.msgId, qEntry.ctx ?? sourceCtxFor(qEntry.sessionId));
      return;
    }
    ...
    // 撤回原审批消息：否则另一个选项仍显示为可点（QQ 无法原地更新键盘）
    if (entry.msgId) recallMessage(entry.msgId, entry.ctx ?? sourceCtxFor(entry.sessionId));
  }
    ...
    if (ev.type === 'approval/decided') {
      ...
      for (const [rid, e] of pendingApprovals) {
        if (e.approvalId === decidedId) {
          ...
          // 一并撤回 QQ 上的审批消息（按钮随即失效）
          if (e.msgId) recallMessage(e.msgId, e.ctx ?? sourceCtxFor(p.sessionId));
          break;
        }
      }
      return;
