...
function runChannel(ctx, config, logger) {
  ...
  async function handleInteraction(d) {
    ...
    // 操作者校验：配置了白名单则必须命中
    if (config.allowedUsers.length > 0 && operator && !config.allowedUsers.includes(operator)) {
      log(`interaction operator ${operator} not in allowlist — ignored`);
      return;
    }
  ...
  function tryAnswerPending(text) {    const t = String(text).trim();
  ...
  function handleFrame(frame) {
    ...
    if (ev.type === 'turn/end') {
      ...
      st.turnBuffer = [];
      // 设计缺陷修复：只有该轮次由 QQ 消息触发（队列里有目标）才回发；
      // GUI/其他客户端触发的轮次绝不广播到 QQ。
      const c = st.turnCtxQueue.shift();
      if (!c) return;
  ...
  async function connectQQ() {
    ...
    while (!disposed.flag) {
      ...
      // close-code 分类重连表（按 QQ WS 规范 + Hermes 语义修订版）
      if (closeCode === 4004) {
        // 鉴权失败：清 token（单飞重取）+ 清 session
        accessToken = '';
        gatewaySessionId = '';
        log('QQ close 4004: clearing access token and session');
      } else if (closeCode === 4008 || closeCode === 4009) {
        // 会话失效：清 session，退避后重新 Identify
        gatewaySessionId = '';
        backoff = Math.min(backoff * 2, 60000);
        log(`QQ close ${closeCode}: clearing session, backoff ${backoff}ms`);
      } else if (closeCode === 4003 || closeCode === 4005) {
        // 网关内部错误：立即重新 Identify（短退避）
        backoff = 1000;
        log(`QQ close ${closeCode}: gateway error, quick re-identify`);
      } else if (closeCode === 4006 || closeCode === 4007 || (closeCode >= 4900 && closeCode <= 4913)) {
        // Resume/Identify 失败：清 session 重新 Identify
        gatewaySessionId = '';
        log(`QQ close ${closeCode}: resume/identify failed, clearing session`);
  ...
  // 启动日志：明确打印实际生效模式与会话来源，便于多 AI 环境下快速诊断
  function bootLine() {
    ...
    return `bridge up: QQ -> DSH session ${targetSessionId} [mode=${mode}, src=${src}]`;
  }
