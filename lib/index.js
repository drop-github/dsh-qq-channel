...
function runChannel(ctx, config, logger) {
  ...
  // 文件日志：DSH_HOME/storages/qq-channel.log（宿主控制台不可见时用它诊断）
  const dshHomeForLog = process.env.DSH_HOME ?? '';
  const LOG_FILE = dshHomeForLog ? path.join(dshHomeForLog, 'storages', 'qq-channel.log') : '';
  const log = (msg) => {
  ...
  function handleQQDispatch(t, d) {
    ...
    log(`QQ message ${t} (${c.groupOpenId ?? c.userOpenId}): ${text.slice(0, 120)}`);
    ...
    rpc('session.prompt', {
      ...
      .then((value) => {
        ...
        if (config.ack) sendText('已收到 ✅ 正在处理…', { groupOpenId: c.groupOpenId, userOpenId: c.userOpenId });
      })
      .catch((err) => {
